...
/// </remarks>
public abstract class CosmosRepositoryBase<T> where T : DocumentEntityBase
{
    ...
    public virtual async Task<T> Find(Expression<Func<T, bool>> predicate)
    {
        ...
        FeedResponse<T> response = await _cosmos.ExecuteIterator(c => c
            .GetItemLinqQueryable<T>(true, null, null)
    ...
    public virtual async Task<List<T>> GetItemsFiltered<TOrder>(int startIndex, int pageSize,
        Expression<Func<T, TOrder>> orderPredicate, SortDirection sortDirection,
        params Expression<Func<T, bool>>[] filterPredicates)
    {
    ...
    public virtual async Task<List<Tresponse>> Query<Tresponse, Tentity>(QueryDefinition query) where Tentity : DocumentEntityBase
    {
